...
const ResultPage = () => {
      ...
      setAudioList(prevList => {
        ...
        if (!isExist) {
          return [{
            ...
            duration: location.state?.fileDuration ? formatTime(location.state.fileDuration) : '00:00',
  ...
  if (isMobile) {
    return <MobileResultPage uploadedId={uploadedId} audioList={audioList} setAudioList={setAudioList} analysisResult={analysisResult} />;
  }

  return (
    <div className="result-page-wrapper">
      <div className="result-page-container">
        {audioList.find(a => a.id === activeAudioId)?.audioUrl && (
          <audio ref={audioRef} src={audioList.find(a => a.id === activeAudioId).audioUrl} onTimeUpdate={() => setCurrentTime(audioRef.current.currentTime)} onLoadedMetadata={() => setDuration(audioRef.current.duration)} onEnded={() => setIsPlaying(false)} />
        )}
        <div className="result-left-panel">
          <div className="audio-list-container">
            {audioList.length > 0 ? (
              audioList.map((audio) => (
                <div key={audio.id} className={`audio-card ${activeAudioId === audio.id ? 'active' : ''}`} onClick={() => handleCardClick(audio.id)}>
                  <span className="audio-name">{audio.name}</span>
                  <span className="audio-duration">{audio.duration}</span>
                </div>
              ))
            ) : (
              <div className="empty-list-notice"><p>음성 파일을 업로드해주세요.</p></div>
            )}
          </div>
          {isLoggedIn && (
